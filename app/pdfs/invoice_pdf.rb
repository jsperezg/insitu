class InvoicePdf < DocumentPdf
  ...
  def footer_totals
    ...
    totals << [
        ...
        data_cell("#{ number_with_precision(@invoice.subtotal, precision: 2) } #{ currency_symbol(@current_user) }", [:top], default_padding, :right)
    ]

    if @invoice.discount > 0
      totals << [
          header_cell("#{ Invoice.human_attribute_name :discount }:", default_borders, default_padding, :right),
          data_cell("#{ number_with_precision(@invoice.discount, precision: 2) } #{ currency_symbol(@current_user) }", default_borders, default_padding, :right)
      ]
    end
    ...
    if @invoice.irpf > 0
      totals << [
          ...
          data_cell("- #{ number_with_precision(@invoice.applied_irpf, precision: 2) } #{ currency_symbol(@current_user) }", default_borders, default_padding, :right)
      ]
    end
