...
class Estimate < ApplicationRecord
  ...
  def rejected?
    (!accepted? && (valid_until.nil? || valid_until <= Date.today)) ||
      estimate_status&.name == 'estimate_status.rejected'
  end
  ...
  def validate_valid_until
    if !valid_until.nil? and !date.nil? and valid_until < date
      errors.add(:valid_until,
                 I18n.t('activerecord.errors.models.estimate.attributes.valid_until.invalid_value'))
    end
  end
  ...
  def number_format
    unless is_number_valid?(self.number, self.date)
      ...
      errors.add(:number,
                 I18n.t('activerecord.errors.models.estimate.attributes.number.invalid_format', year: year ))
    end
  end
end
