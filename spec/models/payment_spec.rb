...
RSpec.describe Payment, type: :model do
  describe 'Validations' do
    describe 'txn_id' do
      ...
      it 'max length: 19' do
        payment = Payment.create(txn_id: (0...20).map { ('a'..'z').to_a[rand(26)] }.join)
        expect(payment.errors).to be_key :txn_id
      end
    end
    ...
    describe 'business' do
      ...
      it 'max length: 127' do
        payment = Payment.create(business: (0...128).map { ('a'..'z').to_a[rand(26)] }.join)
        expect(payment.errors).to be_key :business
      end
    end
    ...
    describe 'receiver_email' do
      ...
      it 'max length: 127' do
        payment = Payment.create(receiver_email: (0...128).map { ('a'..'z').to_a[rand(26)] }.join)
        payment.save
        expect(payment.errors).to be_key :receiver_email
      end

      it 'Valid value must be defined' do
        payment = Payment.create(receiver_email: (0...127).map { ('a'..'z').to_a[rand(26)] }.join)
        expect(payment.errors).to be_key :receiver_email

        payment = Payment.create(receiver_email: Rails.configuration.x.paypal_receiver_email)
    ...
    describe 'receiver_id' do
      ...
      it 'max length: 13' do
        payment = Payment.create(receiver_id: (0...14).map { ('a'..'z').to_a[rand(26)] }.join)
        expect(payment.errors).to be_key :receiver_id
      end
    end
    ...
    describe 'residence_country' do
      it 'max length: 2' do
        payment = Payment.create(residence_country: (0...3).map { ('a'..'z').to_a[rand(26)] }.join)
    ...
    describe 'payer_id' do
      ...
      it 'max length: 13' do
        payment = Payment.create(payer_id: (0...14).map { ('a'..'z').to_a[rand(26)] }.join)
        expect(payment.errors).to be_key :payer_id
      end
    end
    ...
    describe 'payer_email' do
      ...
      it 'max length: 127' do
        payment = Payment.create(payer_email: (0...128).map { ('a'..'z').to_a[rand(26)] }.join)
        expect(payment.errors).to be_key :payer_email
      end
    end
    ...
    describe 'payer_status' do
      ...
      it 'max length: 10' do
        payment = Payment.create(payer_email: (0...11).map { ('a'..'z').to_a[rand(26)] }.join)
        expect(payment.errors).to be_key :payer_status
      end
    end

    it 'last_name max length: 64' do
      payment = Payment.create(last_name: (0...65).map { ('a'..'z').to_a[rand(26)] }.join)
      expect(payment.errors).to be_key :last_name
    end

    it 'first_name max length: 64' do
      payment = Payment.create(first_name: (0...65).map { ('a'..'z').to_a[rand(26)] }.join)
    ...
    describe 'payment_type' do
      ...
      it 'max length: 7' do
        payment = Payment.create(payer_email: (0...8).map { ('a'..'z').to_a[rand(26)] }.join)
        expect(payment.errors).to be_key :payment_type
      end
    end
    ...
    describe 'txn_type' do
      ...
      it 'max length: 50' do
        payment = Payment.create(txn_type: (0...51).map { ('a'..'z').to_a[rand(26)] }.join)
        expect(payment.errors).to be_key :txn_type
      end
    end
    ...
    describe 'mc_gross' do
      ...
      it 'is valid with regards to plan data' do
        ...
        payment = Payment.create(plan_id: plan.id, mc_gross: 1)
        expect(payment.errors).to be_key :mc_gross

        payment = Payment.create(plan_id: plan.id, mc_gross: plan.price * (1 + plan.vat_rate / 100.0))
        expect(payment.save).to be_falsey
    ...
    describe 'quantity' do
      ...
      it 'must be a positive number' do
        payment = Payment.create(quantity: 0)
        expect(payment.errors).to be_key :quantity
      end

      it 'must be an integer' do
        payment = Payment.create(quantity: 0.12)
        expect(payment.errors).to be_key :quantity
      end
    end
    ...
    describe 'mc_currency' do
      ...
      it 'maximum length: 5' do
        payment = Payment.create(mc_currency: (0...6).map { ('a'..'z').to_a[rand(26)] }.join)
        expect(payment.errors).to be_key :mc_currency
      end

      it 'Only EUR accepted' do
        payment = Payment.create(mc_currency: (0...5).map { ('a'..'z').to_a[rand(26)] }.join)
        expect(payment.errors).to be_key :mc_currency

        payment = Payment.create(mc_currency: 'EUR')
        expect(payment.errors).not_to be_key :mc_currency
      end
    end

    it 'notify_version accepts maximum 25 chars' do
      payment = Payment.create(notify_version: (0...26).map { ('a'..'z').to_a[rand(26)] }.join)
